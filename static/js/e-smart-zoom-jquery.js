  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	    ...
	    init : function(options) {
		    ...
		    var zoomContainerId = "smartZoomContainer"+System.currentTimeMillis();
    ...
    function touchEndHandler(e){
    	...
    	if(smartData.touch.touchMove){ // smooth motion at end if we are in drag mode
    		...
    		if(smartData.settings.dblTapEnabled == true && smartData.touch.lastTouchEndTime != 0 && System.currentTimeMillis() - smartData.touch.lastTouchEndTime < 400){ // if the user double tap (double tap if there is less than 300 ms between first and second tap)
				var lastStartPos = smartData.touch.lastTouchPositionArr[0];
			    zoomOnDblClick(lastStartPos.x, lastStartPos.y);  
	    	}
	    	smartData.touch.lastTouchEndTime = System.currentTimeMillis();
    	}
    }
