...
function load() {
    ...
    var editid = sessionStorage.getItem('equipmenteditid');
                ...
                columns: [
                    ...
                    // },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if(mapid ==value){
                                return '<span class ="label label-primary">已同步</span>';
                            }else {
...
//同步操作
function BindProcess(status) {

    //begin

    var data = {};
    var rows = $('#exampleTable').bootstrapTable("getSelections");//获取选中的id
    if (rows.length == 0){
        if(status==1){
            layer.msg("请选择要同步的数据");
        }
        ...
        layer.confirm('确定要同步选中的记录？', {
            ...
            var pId = new Array();
            var robothadwarelist=new Array();

            for (var i = 0; i < rows.length; i++) {
                //pId.push(rows[i].id)
                robothadwarelist.push(rows[i].hardwearNo);
            }

            //data.ids = pId;
            //data.status = status;
            data.mapid=getQueryString('mapid');
            data.robothadwarelist=robothadwarelist;
            //data.robothadware = 555;
            //data.robotid = pId;
            //irobotweb/sys/map/setcurrentmap

            var jsonStr=JSON.stringify(data);
            //console.info('同步的消息jsonStr');console.info(jsonStr);

            $.ajax({
                url: share + '/irobotweb/sys/map/applymap',
                crossDomain: true,
                type: "POST",
                ...
                success: function (data) {
                    ...
                    //console.info('同步返回的消息');console.info(data);
