...
var resource_type = 0;
var welcomeguest_poiname='',welcomeguest_poiid='';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(arr_old!=null && arr_old.length>0){
                $('#pointId').val(arr_old[0].poiId);//点位名称  共用的字段
                $('#point').val(arr_old[0].poiName);//点位名称  共用的字段
                $('#voice').val(arr_old[0].voice);//语音
                $('#emoticon').val(arr_old[0].emoji+arr_old[0].emojiType);//表情
                $('#delayTime').val(arr_old[0].delayTime);//图片播放时间
                ...
                if(arr_old.length >1){
                    ...
                    if (arr_old.length > 1) {
                        ...
                        for (var i = 2; i < arr_old.length; i++) { //注意，此处i的初始值是1，索引为0的项已经在上面使用了
    ...
    if (divs != null && divs.length > 0) {
        for (i = 0; i < divs.length; i++) {
            ...
            if (i == 0) { //i==0 时处理两笔任务数据
                ...
                content_child_first.poiName = $('#point').val();//点位
                content_child_first.poiId = $('#pointId').val();//点位id
                //content_child_first.poiId=current_poi_id;
            ...
            //ontent_child.poiName = $('#point').val();//点位
            ...
            //alert(content_child.emoji_type);
...
function selectPoint() {
    ...
    layer.open({
        ...
        shadeClose: false,
        area: ['900px', '700px'],
        content: './PointSelect.html?poiType=wlcmpoint',  // iframe的url
        end: function () {

            if(welcomeguest_poiid!='' && welcomeguest_poiid!=null){
                $('#point').val(welcomeguest_poiname);
                $('#pointId').val(welcomeguest_poiid);
            }
        },
        cancel:function () {
            //alert(welcomeguest_poiname+666);
        }
    });
}
