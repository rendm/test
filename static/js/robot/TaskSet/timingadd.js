...
$.ajax({
    ...
    success: function (data) {
        console.log(data.rows,'机器人名称')
        var jsonstring = data.rows;
        var options='';
        $.each(jsonstring, function (i, row){
            console.log(i,'ii')//下标
            console.log(row,'rowrow')//每一项数据
            options +='<option mapname="'+row.currentmap+'" hardwearNo="'+row.hardwearNo+'" value="'+row.id+'">'+row.robotName+'</option>'
        });
...
function save()
{
    if (validateRule2().form())
    {
        ...
        $.ajax({
            url : share+"/irobotweb/sys/cronjob/add",
            type: "POST",
            crossDomain: true,
            xhrFields: {
                withCredentials: true
            ...
            data : JSON.stringify(overallSavejson),
            success : function(data){
                console.log(data,'修改参数返回')
                if(data.code==200 || data.code==0){
                    //parent.layer.msg("定时任务添加成功");

                    layer.confirm('定时任务添加成功', {
                        btn: ['确定', '取消']
                    }, function () {
...
function newconstruction(){
    ...
    console.log(ss,'smsmsmsm嘀嗒嘀嗒嘀嗒嘀嗒嘀嗒嘀嗒嘀嗒')
